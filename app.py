...
def load_all_partners():
    ...
    if combined_dfs:
        ...
        # Fallback dummy data structure matching the 5 target districts
        return pd.DataFrame({
...
# Filter by District (Pill-style Buttons)
...
if selected_district != "All Districts":
    filtered_df = filtered_df[filtered_df["District"].str.lower() == selected_district.lower()]
...
achieved_count = len(filtered_df[filtered_df["Status"].astype(str).str.lower().str.contains("achieved|completed")])
in_progress_count = len(filtered_df[filtered_df["Status"].astype(str).str.lower().str.contains("progress")])
not_started_count = len(filtered_df[filtered_df["Status"].astype(str).str.lower().str.contains("not started|pending")])

overall_pct = int((achieved_count / total_records * 100)) if total_records > 0 else 0

# Visual Overall Progress Card (Matching your screenshot)
st.markdown(f"""
...
with tab1:
    ...
    total_target = filtered_df["Target_Beneficiaries"].sum() if "Target_Beneficiaries" in filtered_df else 0
    total_reached = filtered_df["Reached_Beneficiaries"].sum() if "Reached_Beneficiaries" in filtered_df else 0
    total_kits = filtered_df["Kits_Distributed"].sum() if "Kits_Distributed" in filtered_df else 0
    reach_pct = (total_reached / total_target * 100) if total_target > 0 else 0
    
    with col1:
        st.markdown(f'<div class="kpi-card"><div class="kpi-title">Target Beneficiaries</div><div class="kpi-value">{total_target:,}</div></div>', unsafe_allow_html=True)
    with col2:
        st.markdown(f'<div class="kpi-card"><div class="kpi-title">Reached Beneficiaries</div><div class="kpi-value">{total_reached:,}</div></div>', unsafe_allow_html=True)
    with col3:
        st.markdown(f'<div class="kpi-card"><div class="kpi-title">Kits Distributed</div><div class="kpi-value">{total_kits:,}</div></div>', unsafe_allow_html=True)
    with col4:
...
with tab2:
    ...
    st.dataframe(filtered_df[["Partner", "District", "Status"]], use_container_width=True)
